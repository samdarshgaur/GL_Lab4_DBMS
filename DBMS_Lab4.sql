...
*/
select C.cus_id, C.cus_gender from customer as C
inner join
(select cus_id, sum(ord_amount) from `order` group by cus_id having sum(ord_amount) >= 3000) as O
...
*/
select K.ord_id, P.pro_id, P.pro_name from product as P
inner join
(select O.ord_id, SP.pricing_id, SP.pro_id from supplier_pricing as SP
inner join
(select ord_id, pricing_id from `order` where cus_id = 2) as O
on SP.pricing_id = O.pricing_id) as K
on K.pro_id = P.pro_id;
...
inner join
(select supp_id, count(pro_id) from supplier_pricing group by supp_id having count(pro_id) > 1) as SP
...
*/
select category.cat_id, category.cat_name, min(t3.min_price) as Min_Price, t3.pro_name from
category
inner join
